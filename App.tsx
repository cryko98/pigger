  ...
  return (
    <section id="home" className="min-h-screen flex flex-col items-center justify-center pt-20 px-6">
      <motion.div style={{ y: y1, opacity }} className="text-center">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 1.2, ease: "easeOut" }} className="relative inline-block mb-12">
          <div className="absolute inset-0 bg-white/10 blur-[100px] rounded-full" />
          <img src={PIGGER_IMG} alt="Pigger" className="w-48 h-48 md:w-80 md:h-80 rounded-full border-4 border-white/5 relative z-10 animate-float" />
        </motion.div>
        <div className="relative">
          <motion.h1 initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.8 }} className="text-7xl md:text-[10rem] font-syne font-extrabold leading-[0.8] tracking-tighter gradient-text uppercase">WIGGER<br/>PIG</motion.h1>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="mt-8 text-white/30 font-syne text-xs md:text-sm tracking-[0.5em] font-bold uppercase">The Culture Coin on Solana</motion.p>
        </div>
    ...
    <div className="max-w-6xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
        <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
          <h2 className="text-4xl md:text-7xl font-syne font-extrabold mb-10 uppercase gradient-text leading-tight">NOT JUST<br/>A MEME.</h2>
          <div className="space-y-6 text-white/50 font-syne text-lg leading-relaxed">
            ...
            <div className="flex flex-wrap gap-3 mt-10">
              {['0% TAX', 'BURNT LP', 'COMMUNITY DRIVEN', 'MEME POWER'].map((tag) => (
                <span key={tag} className="px-5 py-2 bg-white/5 border border-white/5 rounded-full text-[9px] font-syne font-bold tracking-widest text-white/40 uppercase">{tag}</span>
              ))}
            </div>
          </div>
        </motion.div>
...
const Tokenomics = () => {
  ...
  return (
    <section id="contract" className="py-40 px-6">
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="soft-glass rounded-[4rem] p-8 md:p-24 text-center overflow-hidden relative">
          ...
          <div className="max-w-2xl mx-auto mb-16">
            <div className="bg-black/40 border border-white/5 rounded-3xl p-5 flex flex-col md:flex-row items-center gap-4">
              ...
              <button onClick={handleCopy} className="w-full md:w-auto px-8 py-4 bg-white text-black font-syne font-extrabold text-[10px] tracking-widest rounded-2xl hover:bg-white/90 transition-all uppercase">{copied ? 'COPIED!' : 'COPY ADDR'}</button>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[ { label: "SUPPLY", val: "1,000,000,000" }, { label: "BUY/SELL TAX", val: "0% TAX" }, { label: "LIQUIDITY", val: "BURNT" } ].map((stat, i) => (
              <motion.div key={i} whileHover={{ y: -10 }} className="p-10 border border-white/5 rounded-[2.5rem] bg-white/[0.01] flex flex-col items-center">
                <p className="text-[9px] font-syne text-white/20 tracking-[0.3em] font-bold mb-4 uppercase">{stat.label}</p>
                <p className="text-xl md:text-2xl font-syne font-extrabold text-white uppercase">{stat.val}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
          ...
          <div className="lg:w-1/3 space-y-10">
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => setPrompt(randomScenarios[Math.floor(Math.random() * randomScenarios.length)])} className="text-[9px] font-syne font-bold text-white/60 hover:text-white transition-colors uppercase">RANDOMIZE</button>
              </div>
              <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="Pigger riding a gold-plated jet..." className="w-full h-32 bg-white/[0.02] border border-white/10 rounded-3xl p-6 text-white font-syne text-xs outline-none focus:border-white/20 transition-all resize-none" />
            </div>
            <div className="space-y-4">
              <label className="font-syne text-[10px] text-white/30 tracking-widest font-bold uppercase">SELECT STYLE</label>
              <div className="flex flex-wrap gap-2">
                {stylePresets.map(p => (
                  <button key={p.id} onClick={() => setSelectedPreset(p.id)} className={`px-4 py-2 rounded-full font-syne text-[9px] font-bold tracking-widest transition-all uppercase ${selectedPreset === p.id ? 'bg-white text-black' : 'bg-white/5 text-white/40 hover:bg-white/10'}`}>{p.label}</button>
                ))}
              </div>
            </div>
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {[
          ...
          <motion.a key={i} href={link.url} target="_blank" whileHover={{ scale: 0.98 }} className="flex items-center justify-between p-12 soft-glass rounded-[3rem] hover:bg-white/[0.05] transition-all group">
            <div>
            ...
            <span className="text-2xl opacity-0 group-hover:opacity-100 transition-opacity uppercase">→</span>
          </motion.a>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen selection:bg-white selection:text-black">
      <AnimatePresence>
        {!isLoaded && (
          <motion.div key="preloader" exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-[#050505]">
            <motion.img animate={{ scale: [1, 1.1, 1], opacity: [0.5, 1, 0.5] }} transition={{ repeat: Infinity, duration: 2 }} src={PIGGER_IMG} className="w-20 h-20 rounded-full mb-6" />
      ...
      <div className="fixed inset-0 -z-10 pointer-events-none opacity-20 overflow-hidden">
        <motion.div animate={{ x: [0, 100, 0], y: [0, 50, 0] }} transition={{ duration: 20, repeat: Infinity }} className="absolute -top-[10%] -left-[10%] w-[50vw] h-[50vw] bg-white/5 blur-[120px] rounded-full" />
        <motion.div animate={{ x: [0, -100, 0], y: [0, -50, 0] }} transition={{ duration: 25, repeat: Infinity }} className="absolute -bottom-[10%] -right-[10%] w-[40vw] h-[40vw] bg-white/5 blur-[120px] rounded-full" />
      </div>
    </div>
  );
};
